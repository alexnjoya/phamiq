...
async def test_llm_endpoint():
    ...
    try:
        if not alleai_service.is_available():
            return {
                "status": "warning",
                "message": "AlleAI not available - API key not configured",
                "llm_available": False
            }
...
async def get_disease_recommendations(
    ...
    try:
        if not alleai_service.is_available():
            raise HTTPException(
                status_code=503,
                detail="LLM service is required but not available. Please configure AlleAI API key."
            )
        
        # Get comprehensive LLM recommendations
        recommendations_data = await alleai_service.get_disease_recommendations(
            disease_name, 
            confidence, 
            crop_type,
            models
...
async def get_cache_stats():
    ...
    try:
        stats = alleai_service.get_cache_stats()
        return {
            "status": "success",
            "cache_stats": stats
        }
...
async def predict_disease(
    ...
    try:
        ...
        if not alleai_service.is_available():
            logger.error("AlleAI service not available - LLM is required for disease analysis")
            raise HTTPException(
                status_code=503,
                detail="LLM service is required for disease analysis but not available. Please configure AlleAI API key."
            )
        ...
        # Save to history
        try:
            history_entry = PredictionHistoryModel(
